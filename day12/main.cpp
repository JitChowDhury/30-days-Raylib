...
void SpawnEnemies(std::vector<Enemy> &enemies, int &numEnemies)
{
  for (int i = 0; i < numEnemies; i++)
  {
    ...
    enemies.emplace_back(Vector2{static_cast<float>(rand() % (maxPos - minPos + 1) + minPos), static_cast<float>((rand()) % (maxPos - minPos + 1) + minPos)}, Vector2{static_cast<float>(rand() % (maxPos - minPos + 1) + minPos), static_cast<float>((rand()) % (maxPos - minPos + 1) + minPos)}, static_cast<float>(rand() % (30 - 10 + 1) + 10));
  }
...
int main()
{
  ...
  InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "Day 11 Game");
